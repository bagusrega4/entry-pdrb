...
class RasioSheetImport implements ToCollection, WithHeadingRow, SkipsEmptyRows
{
    private RasioImport $parent;

    private array $commodityCache = [];
    private array $triwulanCache  = [];

    public function __construct(RasioImport $parent)
    {
        $this->parent = $parent;
    }
    ...
    public function collection(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $rasioOutputIkutan = $this->cleanNumber($row['rasio_output_ikutan'] ?? null);
            $rasioWipCbr       = $this->cleanNumber($row['rasio_wip_cbr'] ?? null);
            $rasioBiayaAntara  = $this->cleanNumber($row['rasio_biaya_antara'] ?? null);

            if ($rasioOutputIkutan === null && $rasioWipCbr === null && $rasioBiayaAntara === null) {
                $this->parent->skipped++;
                continue;
            }
            ...
            try {
                CommodityRasio::updateOrCreate(
                    ...
                    [
                        'rasio_output_ikutan' => $rasioOutputIkutan,
                        'rasio_wip_cbr'       => $rasioWipCbr,
                        'rasio_biaya_antara'  => $rasioBiayaAntara,
                    ]
                );
