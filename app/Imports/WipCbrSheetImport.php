...
class WipCbrSheetImport implements ToCollection, WithHeadingRow, SkipsEmptyRows
{
    private WipCbrImport $parent;

    private array $commodityCache = [];
    private array $triwulanCache  = [];

    public function __construct(WipCbrImport $parent)
    {
        $this->parent = $parent;
    }
    ...
    public function collection(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $luasTanam   = $this->cleanNumber($row['luas_tanam'] ?? null);
            $biayaRawat  = $this->cleanNumber($row['biaya_perawatan'] ?? null);

            // skip kalau kosong semua
            if ($luasTanam === null && $biayaRawat === null) {
                $this->parent->skipped++;
                continue;
            }
            ...
            try {
                CommodityWipCbr::updateOrCreate(
                    ...
                    [
                        'luas_tanam_akhir_tahun' => $luasTanam,
                        'biaya_perawatan'        => $biayaRawat,
                    ]
                );
