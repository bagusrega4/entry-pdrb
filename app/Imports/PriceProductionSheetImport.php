...
class PriceProductionSheetImport implements ToCollection, WithHeadingRow, SkipsEmptyRows
{
    private PriceProductionImport $parent;

    private array $commodityCache = [];
    private array $triwulanCache  = [];

    public function __construct(PriceProductionImport $parent)
    {
        $this->parent = $parent;
    }
    ...
    public function collection(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $harga       = $this->cleanNumber($row['harga']      ?? null);
            $produksi    = $this->cleanNumber($row['produksi']   ?? null);

            if ($harga === null && $produksi === null) {
                $this->parent->skipped++;
                continue;
            }
