...
<style>
    * { box-sizing: border-box; }
    ...
    #hargaTable thead th:first-child { text-align: left; }

    #hargaTable tbody tr:hover td { background: #fffbf7; }
    #hargaTable tbody tr:not(:last-child) td { border-bottom: 0.5px solid #f3f4f6; }
    #hargaTable tbody td {
        padding: 8px 12px;
        color: #374151;
    ...
    #hargaTable tbody td.td-name { font-weight: 500; color: #111827; min-width: 220px; }
    #hargaTable tbody td.td-name.fw-bold { color: #111827; background: #f9fafb; }
    #hargaTable tbody td.bg-light { background: #f9fafb !important; }

    /* ── Table inputs ── */
    #hargaTable input.form-control {
        height: 32px;
        padding: 0 8px;
        font-size: 12px;
        font-family: 'Figtree', sans-serif;
    ...
    #hargaTable input.form-control:focus {
        border-color: #f97316;
        box-shadow: 0 0 0 2px rgba(249,115,22,0.1);
    }
    #hargaTable input.form-control::placeholder { color: #d1d5db; }
...
<div class="page">
    ...
    <h3 class="fw-bold mb-1" style="font-family:'Figtree',sans-serif;">Input Harga & Produksi</h3>
    ...
    <div class="card" style="margin-bottom:0;">
        ...
        <div class="card-head" id="table-actions" style="display:none;">
            <div class="card-head-left">
                ...
                <span class="card-head-title">Harga & Produksi Komoditas</span>
            </div>
            <div class="card-head-right">
        ...
        <div id="table-container" style="display:none;">
            <div class="table-wrap">
                <table id="hargaTable">
                    <thead>
                        <tr>
                            <th style="min-width:240px; text-align:left;">Komoditas</th>
                            <th style="width:140px;">Indikator</th>
                            <th style="width:110px;">Sat. Harga</th>
                            <th style="width:120px;">Sat. Produksi</th>
                        </tr>
                    </thead>
                    <tbody></tbody>
                </table>
        ...
        <div id="empty-table-state">
            <div class="empty-state">
                ...
                <p>Pilih <strong>komoditas</strong> di atas untuk menampilkan tabel harga dan produksi.</p>
            </div>
        </div>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Import Data Harga & Produksi</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
...
<script>
    ...
    $(function () {
        ...
        function parseNumberID(formatted) {
            ...
            let s = String(formatted).trim().replace(/Rp\s*/i, '').replace(/\s+/g, '');
        ...
        function formatNumberID(value, isCurrency = false) {
            ...
            if (num === null) return '';
            const formatted = new Intl.NumberFormat('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 10 }).format(num);
        ...
        function buildTable() {
            ...
            const $subRow = $('<tr></tr>').html(`
                <th></th><th></th><th></th><th></th>
            `);
            selectedYears.forEach(() => {
                $subRow.append(`
                    <th class="text-center year-sub-head">Harga</th>
                    <th class="text-center year-sub-head">Produksi</th>
                `);
            });

            $('#hargaTable thead').append($theadRow).append($subRow);
                        ...
                        row += `
                            <td class="${tdClass}" style="padding:6px 8px;">
                                <input type="text" class="form-control harga"
                                    ...
                                    data-raw="${hargaVal}" value="${formatNumberID(hargaVal, true)}" placeholder="Harga">
                            </td>
                            <td class="${tdClass}" style="padding:6px 8px;">
                                <input type="text" class="form-control produksi"
                                    data-year="${y.year}" data-triwulan="${triId}" data-id="${it.id}"
                                    data-raw="${produksiVal}" value="${formatNumberID(produksiVal)}" placeholder="Produksi">
                            </td>`;
                    } else {
                        row += `
                            <td class="${tdClass}" style="text-align:right; font-family:monospace; font-size:12px;">${formatNumberID(hargaVal, true)}</td>
                            <td class="${tdClass}" style="text-align:right; font-family:monospace; font-size:12px;">${formatNumberID(produksiVal)}</td>`;
                    }
                });
        ...
        function attachFormatHandlers() {
            ...
            $('#hargaTable').on('focus', 'input', function () {
                ...
                $(this).data('raw', raw);
                $(this).val(formatNumberID(raw, $(this).hasClass('harga')));
            ...
            $.get('/prices-productions/triwulans').done(triwulans => {
                Swal.fire({
                    ...
                    if (allYears.some(y => y.year === newYear && y.triwulan_id === triwulanId)) {
                        Swal.fire('Gagal', 'Periode sudah ada!', 'error'); return;
                    }
        ...
        $('#saveAll').on('click', function () {
            ...
            $('#hargaTable tbody tr').each(function () {
                ...
                $(this).find('input.harga, input.produksi').each(function () {
                    ...
                    if ($(this).hasClass('harga')) inputData[key].harga = raw;
                    else inputData[key].produksi = raw;
                });
            ...
            if (!payload.length) { Swal.fire('Info', 'Tidak ada data untuk disimpan', 'info'); return; }

            $.post('/prices-productions/bulk-store', { data: JSON.stringify(payload) })
                .done(() => {
        ...
        $('#addCommodity').on('click', function () {
            ...
            setupDropdown('#newIndikator', '/prices-productions/indicators', 'Indikator', i => $('#newIndikator').append(`<option value="${i.id}">${i.indikator}</option>`));
            setupDropdown('#newSatuanHarga', '/prices-productions/unit-harga', 'Satuan Harga', u => $('#newSatuanHarga').append(`<option value="${u.id}">${u.satuan_harga}</option>`));
            setupDropdown('#newSatuanProduksi', '/prices-productions/unit-produksi', 'Satuan Produksi', u => $('#newSatuanProduksi').append(`<option value="${u.id}">${u.satuan_produksi}</option>`));
            setupDropdown('#newSatuanLuasTanam', '/prices-productions/unit-luas', 'Satuan Luas Tanam', u => $('#newSatuanLuasTanam').append(`<option value="${u.id}">${u.satuan_luas_tanam}</option>`));
            setupDropdown('#newSatuanBiayaPerawatan', '/prices-productions/unit-perawatan', 'Satuan Biaya Perawatan', u => $('#newSatuanBiayaPerawatan').append(`<option value="${u.id}">${u.satuan_biaya_perawatan}</option>`));
        ...
        ['#newIndikator','#newSatuanHarga','#newSatuanProduksi','#newSatuanLuasTanam','#newSatuanBiayaPerawatan'].forEach(selector => {
            $(selector).on('change', function () {
                ...
                const typeMap = {
                    ...
                    '#newSatuanLuasTanam': { title: 'Satuan Luas Tanam', endpoint: '/prices-productions/unit-luas', field: 'satuan_luas_tanam' },
                    '#newSatuanBiayaPerawatan': { title: 'Satuan Biaya Perawatan', endpoint: '/prices-productions/unit-perawatan', field: 'satuan_biaya_perawatan' }
                };
        ...
        $('#parentSelect').on('change', function () {
            ...
            const endpoint = parentId ? `/prices-productions/commodities/${parentId}/next-code` : `/prices-productions/commodities/next-code`;
        ...
        $('#saveCommodity').on('click', function () {
            ...
            if (!data.kode || !data.nama) { Swal.fire('Error', 'Kode dan nama wajib diisi', 'error'); return; }
        ...
        $('#level1').on('change', function () { currentRootId = $(this).val(); });

        $('#uploadDoc').on('click', function () {
            if (!currentRootId) { Swal.fire('Perhatian', 'Pilih komoditas terlebih dahulu!', 'warning'); return; }
        ...
        $('#saveDoc').on('click', function () {
            if (!$('#docName').val()) { Swal.fire('Error', 'Nama dokumen wajib diisi', 'error'); return; }
            if (!$('#docYear').val()) { Swal.fire('Error', 'Tahun wajib diisi', 'error'); return; }
            if (!$('#docFile')[0].files.length) { Swal.fire('Error', 'File wajib diupload', 'error'); return; }
            ...
            formData.append('modul', 'harga_produksi');
            ...
            $.ajax({
                ...
                error: () => Swal.fire('Error', 'Gagal upload dokumen', 'error')
            });
        });
    });

    $('#downloadDoc').on('click', function () {
        fetch('/prices-productions/template')
            .then(r => r.blob())
            .then(blob => {
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url; a.download = 'template_harga_produksi.xlsx';
                document.body.appendChild(a); a.click(); a.remove();
                window.URL.revokeObjectURL(url);
            }).catch(() => Swal.fire('Error', 'Gagal download template', 'error'));
    });
    ...
    $('#submitImport').on('click', function () {
        ...
        if (!file) { Swal.fire('Error', 'File wajib dipilih', 'error'); return; }
        let formData = new FormData();
        formData.append('file', file);
        Swal.fire({ title: 'Mengimport...', text: 'Mohon tunggu sebentar.', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        $.ajax({
            url: '/prices-productions/import', method: 'POST', data: formData, processData: false, contentType: false,
            success: function (res) {
                $('#importModal').modal('hide'); $('#importFile').val('');
                let html = `<b>${res.imported}</b> baris berhasil diimport.<br>`;
                if (res.skipped > 0) html += `<small class="text-muted">${res.skipped} baris dilewati.</small>`;
            ...
            error: function (xhr) {
                let msg = xhr.responseJSON?.message || 'Gagal import data.';
                let errHtml = `<p>${msg}</p>`;
