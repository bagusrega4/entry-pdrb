...
<div class="container">
    <div class="page-inner">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
            <div>
                <h3 class="fw-bold mb-3">Input Harga Komoditas</h3>
                <h6 class="op-7 mb-2">Silakan pilih komoditas lalu isi harga</h6>
            </div>
        </div>
        ...
        <!-- Form harga -->
        <div id="form-harga" style="display:none;">
            <h5>Input Harga</h5>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <small class="form-text text-muted">Isi tahun harga komoditas, default tahun berjalan.</small>
            </div>
            <div class="mb-3">
                <label>Harga Komoditas per Satuan Menurut Tahun</label>
                <input type="number" id="harga" class="form-control">
                <small class="form-text text-muted">Isi harga komoditas berdasarkan indikator & satuan yang dipilih.</small>
            </div>
...
<script>
    $(function() {
        ...
        function loadChildren(parentId, level) {
            $.get('/prices/commodities/' + parentId + '/children', function(data) {
        ...
        function loadIndicatorsAndUnits() {
            // indikator
            $.get('/prices/indicators', function(data) {
                let $indikator = $('#indikator').empty().append($('<option>').val('').text('-- Pilih --'));
                (data || []).forEach(function(i) {
                    $indikator.append($('<option>').val(i.id).text(i.indikator_harga));
                });
            }).fail(function() {
            ...
            // units
            $.get('/prices/units', function(data) {
                let $unit = $('#unit').empty().append($('<option>').val('').text('-- Pilih --'));
                (data || []).forEach(function(u) {
                    $unit.append($('<option>').val(u.id).text(u.satuan_harga));
                });
            }).fail(function() {
        ...
        $('#save').on('click', function() {
            ...
            if (!harga) return Swal.fire('Peringatan', 'Isi harga.', 'warning');
            ...
            if (!indicatorId || !unitId) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        saveHarga(commodityId, indicatorId, unitId, tahun, harga);
                    }
                });
            } else {
                // langsung simpan
        ...
        function saveHarga(commodityId, indicatorId, unitId, tahun, harga) {
            $.post('/prices', {
                _token: '{{ csrf_token() }}',
                commodity_id: commodityId,
                indicator_price_id: indicatorId || null,
                unit_price_id: unitId || null,
                tahun: tahun,
                harga: harga
            }).done(function(res) {
                Swal.fire('Berhasil', 'Harga berhasil disimpan', 'success');
                $('#harga').val('');
                $('#indikator').val('');
                $('#unit').val('');
            }).fail(function(xhr) {
