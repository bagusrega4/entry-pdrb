...
<style>
    * { box-sizing: border-box; }
    ...
    #rasioTable thead th:first-child { text-align: left; }

    #rasioTable tbody tr:hover td { background: #fffbf7; }
    #rasioTable tbody tr:not(:last-child) td { border-bottom: 0.5px solid #f3f4f6; }
    #rasioTable tbody td {
        padding: 8px 12px;
        color: #374151;
    ...
    #rasioTable tbody td.td-name { font-weight: 500; color: #111827; min-width: 220px; }
    #rasioTable tbody td.td-name.fw-bold { color: #111827; background: #f9fafb; }
    #rasioTable tbody td.bg-light { background: #f9fafb !important; }

    /* Table inputs */
    #rasioTable input.form-control {
        height: 32px;
        padding: 0 8px;
        font-size: 12px;
        font-family: 'Figtree', sans-serif;
    ...
    #rasioTable input.form-control:focus {
        border-color: #f97316;
        box-shadow: 0 0 0 2px rgba(249,115,22,0.1);
    }
    #rasioTable input.form-control::placeholder { color: #d1d5db; }
...
<div class="page">
    ...
    <h3 style="font-family:'Figtree',sans-serif; font-size:20px; font-weight:600; color:#111827; margin-bottom:4px;">Input Rasio</h3>
    ...
    <div class="card" style="margin-bottom:0;">
        ...
        <!-- Table Actions (hidden until commodity selected) -->
        <div class="card-head" id="table-actions" style="display:none;">
            <div class="card-head-left">
                <svg class="card-head-icon" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M3 15h18M9 3v18"/>
                </svg>
                <span class="card-head-title">Rasio per Komoditas</span>
            </div>
            <div class="card-head-right">
        ...
        <div id="empty-table-state">
            <div class="empty-state">
                ...
                <p>Pilih <strong>komoditas</strong> di atas untuk menampilkan tabel rasio.</p>
            </div>
        </div>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Import Data Rasio</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
...
<script>
    ...
    $(function () {
        ...
        function buildTable() {
            ...
            selectedYears.forEach(y => {
                const label = y.triwulan_name ? `${y.year} - ${y.triwulan_name}` : y.year;
                $theadRow.append(`<th colspan="3" class="text-center year-col-head">${label}</th>`);
            });

            const $subRow = $('<tr></tr>').html(`<th></th><th></th><th></th>`);
            selectedYears.forEach(() => {
                $subRow.append(`
                    <th class="text-center year-sub-head">Output Ikutan</th>
                    <th class="text-center year-sub-head">WIP/CBR</th>
                    <th class="text-center year-sub-head">Biaya Antara</th>
                `);
            });

            $('#rasioTable thead').append($theadRow).append($subRow);
                    ...
                    if (isLeafNode) {
                        row += `
                            <td class="${tdClass}" style="padding:6px 8px;">
                                <input type="text" class="form-control rasio-output-ikutan"
                                    data-year="${y.year}" data-triwulan="${triId}" data-id="${it.id}"
                                    data-raw="${outputVal}" value="${formatNumberID(outputVal)}" placeholder="Output Ikutan">
                            </td>
                            <td class="${tdClass}" style="padding:6px 8px;">
                                <input type="text" class="form-control rasio-wip-cbr"
                                    data-year="${y.year}" data-triwulan="${triId}" data-id="${it.id}"
                                    data-raw="${wipVal}" value="${formatNumberID(wipVal)}" placeholder="WIP/CBR">
                            </td>
                            <td class="${tdClass}" style="padding:6px 8px;">
                                <input type="text" class="form-control rasio-biaya-antara"
                                    data-year="${y.year}" data-triwulan="${triId}" data-id="${it.id}"
                                    data-raw="${antaraVal}" value="${formatNumberID(antaraVal)}" placeholder="Biaya Antara">
                            </td>`;
                    } else {
                        row += `
                            <td class="${tdClass}" style="text-align:right; font-family:monospace; font-size:12px;">${formatNumberID(outputVal)}</td>
                            <td class="${tdClass}" style="text-align:right; font-family:monospace; font-size:12px;">${formatNumberID(wipVal)}</td>
                            <td class="${tdClass}" style="text-align:right; font-family:monospace; font-size:12px;">${formatNumberID(antaraVal)}</td>`;
                    }
                });
        ...
        $('#saveAll').on('click', function () {
            ...
            $('#rasioTable tbody tr').each(function () {
                ...
                $(this).find('input.rasio-output-ikutan, input.rasio-wip-cbr, input.rasio-biaya-antara').each(function () {
                    ...
                    if ($(this).hasClass('rasio-output-ikutan')) inputData[key].rasio_output_ikutan = raw;
                    else if ($(this).hasClass('rasio-wip-cbr')) inputData[key].rasio_wip_cbr = raw;
                    else if ($(this).hasClass('rasio-biaya-antara')) inputData[key].rasio_biaya_antara = raw;
                });
            ...
            $.post('/rasio/bulk-store', { data: JSON.stringify(payload) })
                .done(() => {
        ...
        $('#addCommodity').on('click', function () {
            ...
            setupDropdown('#newIndikator', '/rasio/indicators', 'Indikator', i => $('#newIndikator').append(`<option value="${i.id}">${i.indikator}</option>`));
            setupDropdown('#newSatuanHarga', '/rasio/unit-harga', 'Satuan Harga', u => $('#newSatuanHarga').append(`<option value="${u.id}">${u.satuan_harga}</option>`));
            setupDropdown('#newSatuanProduksi', '/rasio/unit-produksi', 'Satuan Produksi', u => $('#newSatuanProduksi').append(`<option value="${u.id}">${u.satuan_produksi}</option>`));
            setupDropdown('#newSatuanLuasTanam', '/rasio/unit-luas', 'Satuan Luas Tanam', u => $('#newSatuanLuasTanam').append(`<option value="${u.id}">${u.satuan_luas_tanam}</option>`));
            setupDropdown('#newSatuanBiayaPerawatan', '/rasio/unit-perawatan', 'Satuan Biaya Perawatan', u => $('#newSatuanBiayaPerawatan').append(`<option value="${u.id}">${u.satuan_biaya_perawatan}</option>`));
        ...
        $('#parentSelect').on('change', function () {
            ...
            const endpoint = parentId ? `/rasio/commodities/${parentId}/next-code` : `/rasio/commodities/next-code`;
    ...
    $('#downloadDoc').on('click', function () {
        fetch('/rasio/template')
            .then(r => r.blob())
            .then(blob => {
                const url = window.URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url; a.download = 'template_rasio.xlsx';
    ...
    $('#submitImport').on('click', function () {
        ...
        $.ajax({
            url: '/rasio/import', method: 'POST', data: formData, processData: false, contentType: false,
